...
function TrendingProducts({route, navigation}) {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      setLoading(true);
      axios
        .get(`${config.BACKEND_URI}/api/app/get/all/trending/products`, {
          headers: {
  ...
  const loadMoreItems = async () => {
    if (currentPage < totalPagesCount) {
      ...
      axios
        .get(
          `${config.BACKEND_URI}/api/app/get/all/trending/products?&page=${
            currentPage + 1
          }`,
          {
            headers: {
    ...
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <Surface style={styles.productHeader}>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          ...
          />
          <Text style={styles.searchResultText}>Trending Products</Text>
        </View>
