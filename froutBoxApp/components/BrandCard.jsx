  ...
  <View>
      <View style={styles.brandContainer}>
       ...
       {/*=== FIRST FOUR ==== */}
        ...
        <TouchableOpacity activeOpacity={0.6} onPress={()=>navigation.navigate(navigationString.NEW_ARRIVALS,{searchValue:'t',category:title})}  style={styles.brandBox}  >
             <Image
       ...
       <AnimatedPulse numOfPulse={1} diameter={130} color={config.animationColor} />
        </View>
       <Text style={{...styles.brandName}} >New Arrivals</Text>
          ...
          {brandsData?.map((value,index)=>
          {
            if (index < 3)
           return (
              <TouchableOpacity activeOpacity={0.7} onPress={()=>goToSubCategory(value.brandName,value.category_id)} style={styles.brandBox} key={index} >
             <Image
        style={{width:50,height:50}}
         source={{
       ...
       />
       <Text style={styles.brandName} >{value.brandName?.slice(0,10)}</Text>
        </TouchableOpacity >
          )
          }
          )}
