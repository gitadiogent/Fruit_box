...
export default function () {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      if (slug.length !== 0) {
        const url = `${baseUrl}/api/website/front/search/in/products?page=1&size=12&search=${slug}`;
        ...
        const url_pro = `${baseUrl}/api/website/front/all/products`;
  ...
  async function PageData(page) {
    try {
      setLoading(true);
      const url = `${baseUrl}/api/website/front/search/in/products?page=${page}&size=12&search=${slug}`;
      const res = await axios.get(url, { withCredentials: true });
      console.log(res);
      setCount(res.data?.count);
