...
export default function SliderTwo({ className, data, showArrows, showDots }) {
  ...
  const settings = {
    ...
    slidesToScroll: 1,
    fade: true,
    prevArrow: <div className="nextArrowSlider"><GrFormPrevious/></div>,
    nextArrow: <div className="nextArrowSlider"><GrFormNext /></div>,
    arrows: true,
        ...
        <Slider {...settings}>
          {data.map((slide, index) => (
            <div
              ...
              key={index}
            >
              <Link href={slide?.selected_category ? `/shop/category/${slide?.selected_category}` : "/"}>
