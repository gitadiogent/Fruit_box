...
export default function NewArrival({ data, mainHeading, description }) {
  const responsive = {
    superLargeDesktop: {
  ...
  const fetchData = async () => {
    try {
      const url = `${baseUrl}/api/website/front/get/product/arrivals`;

      const res = await axios.get(url, { withCredentials: true });

      console.log("fkjsakldjklsadjflk;djfl >>>>>>>>>", res.data);
        ...
        <div className="container mt-5 p-0">
          <div className="brand-two">
            <div className="two d-flex justify-content-between w-100 px-2">
              ...
              <Link
                 href={`${process.env.PUBLIC_URL}/new-arrivals`}
                 // href={`#`}NewArrival
                 as={`${process.env.PUBLIC_URL}/new-arrivals`}
                 // as={`#`}
                >
