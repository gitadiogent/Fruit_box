      ...
      {numSelected > 0 ? (
        ...
        >
          <Typography
            ...
            component="div"
          >
            {`All Web Banners (${props.countVendor})`}
          </Typography>
          <Button
...
export default function WebBanner() {
  ...
  const [openImageGuidePopupModal, setOpenImageGuidePopupModal] =
    useState(false);
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/web/banner/get/all/banners`,
        {
          headers: {
  ...
  const handleRemoveBannerImage = async (
    ...
    await axios
      .patch(
        `${process.env.REACT_APP_BACKEND_URL}/api/web/banner/delete/banner/by/id/${bannerId}`,
        {},
        {
          headers: {
            Authorization: `token ${editable_config.FRONTEND_VALIDATOR}`,
  ...
  const addNewBanner = async (e) => {
    console.log(
      "e.target.files e.target.files e.target.files ==========>>>>>>>>",
      e.target.files
    );
    ...
    const bannersToFirebase = await uploadFileToFirebase(
      `/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/users/${authState?.user?.app_id}/banners/${e.target?.files[0]?.name}/`,
      e.target.files[0]
    );
    ...
    if (bannersToFirebase.image_url) {
      await axios
        .post(
          `${process.env.REACT_APP_BACKEND_URL}/api/web/banner/add/new/banner`,
          { ...bannersToFirebase },
          {
            headers: {
              Authorization: `token ${editable_config.FRONTEND_VALIDATOR}`,
  ...
  const handleFileUpload = async (bannerId, e) => {
    ...
    if (bannersToFirebase.image_url) {
      await axios
        .patch(
          `${process.env.REACT_APP_BACKEND_URL}/api/web/banner/change/banner/by/id/${bannerId}`,
          { ...bannersToFirebase },
          {
            headers: {
              Authorization: `token ${editable_config.FRONTEND_VALIDATOR}`,
  ...
  const handleUpdateBannerCategory = async (banner_id, category) => {
    ...
    await axios
      .patch(
        `${process.env.REACT_APP_BACKEND_URL}/api/web/banner/link/banner/to/category/by/id/${banner_id}`,
      ...
      .then((res) => {
        ...
        if (res?.data?.status) {
          setMessage((prev) => ({
            ...prev,
            type: "success",
            message: "Banner Link Change Successfully !",
          }));
          setSnackbarOpen(true);
          setRender((prev) => !prev);
          setLoading(false);
      ...
      .catch((err) => {
        ...
        setMessage((prev) => ({
          ...prev,
          type: "error",
          message: "Banner Link Not Changed !!",
        }));
        setSnackbarOpen(true);
        setRender((prev) => !prev);
        setLoading(false);
  ...
  return (
    <>
      ...
      <PopupModal
        ...
        data={
          <ImageError
            ...
            message="Max banner upload size is 1 MB.  For more read our image upload guidelines."
          />
        }
      />
      {/*===== IMAGE SIZE ERROR Popup Modal ====== */}
      {/*===== Popup Modal ====== */}
      <PopupModal
        handleClose={handleCloseImageGuidelinePopupModal}
        open={openImageGuidePopupModal}
        data={
          <ImageGuidelines handleClose={handleCloseImageGuidelinePopupModal} />
        }
      />
      {/*===== Popup Modal ====== */}
                      ...
                      return (
                        ...
                        /> */}
                            ...
                            >
                              ...
                              <img
                                className="banner-table-image web_banner_table_image"
                                alt="product"
                                src={
                                  row?.image_url ? `${row?.image_url}` : noImage
                                }
                              />
                              {/* </a> */}
                            ...
                            <TableCell
                              ...
                              align="left"
                            >
                              {returnFileName(row?.image_name)?.slice(0, 15)}
                            </TableCell>

                            <TableCell align="center">
                              <p>{convertDate(row?.updatedAt)}</p>
                            </TableCell>
