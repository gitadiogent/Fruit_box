...
import { deleteImageFromFirebase } from "src/global/globalFunctions";
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography
          ...
          component="div"
        >
          All Categories {`(${props.countCategory})`}
        </Typography>
      )}
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/get/all/category?page=${currentPage}&search=${search}&main_category=${filterMainCategory}`,
        {
          headers: {
  ...
  const handleSearch = (e) => {
    ...
    setCurrentPage(1);
    // setLoading(true)
    // axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/search/in/category?search=${search}`,{withCredentials:true})
    // .then(res=>{
    //   console.log(res)
  ...
  const handleDleteCategories = async (value) => {
    ...
    await axios
      ...
      .then(async (res) => {
        // console.log(res)
        console.log(
          "res?.data?.image_need_to_delete With inner object ==>>",
          res?.data?.image_need_to_delete[0]?.main_category_image
        );
        ...
        if (res?.data?.status) {
          ...
          for (let i = 0; i < res?.data?.image_need_to_delete?.length; i++) {
            console.log(
              "res?.data?.image_need_to_delete[i]?.main_category_image?.path=>",
              res?.data?.image_need_to_delete[i]?.main_category_image
            );
  ...
  const goToEditCategoryPage = (cat_id) => {
    navigate(`/dashboard/edit/category/${cat_id}`);
  };
    ...
    <>
     ...
     Add Category
              ...
              <div className="category-topbar-btn">
                ...
                {/* <SideDrawer state={drawerEditMainCategory} toggleDrawerClose={handleCloseEditMainCategorySideBar} toggleDrawerOpen={handleOpenEditMainCategorySidebar}
           ComponentData={<EditMainCategory handleClose={handleCloseEditMainCategorySideBar} />}
           ComponentButton={<Button  variant="contained"  startIcon={<Iconify icon="carbon:category" />}> 
          Main Category
               </Button>} /> */}
                {/*################ MAIN CATEGORY SIDEBAR BUTTON ################*/}

                {/*################ ADD CATEGORY SIDEBAR BUTTON ################*/}
                {/* <SideDrawer state={drawerAddCategory} toggleDrawerClose={handleCloseAddCategorySideBar} toggleDrawerOpen={handleOpenAddCategorySidebar}
           ComponentData={<AddCategory handleClose={handleCloseAddCategorySideBar} />}
           ComponentButton={<Button className='add_Category_page_btn' variant="outlined"  startIcon={<Iconify icon="eva:plus-fill" />}> 
           Add Category
               </Button>} /> */}
                <Button
                  onClick={() => navigate("/dashboard/categories/add/new")}
                  className="add_Category_page_btn"
                  variant="contained"
            ...
            <ConfimModal
              ...
              message="Are you sure you want to delete categories?"
        ...
        <MenuItem component={Link} to="#" sx={{ color: 'text.secondary' }}>
          ...
          <Paper elevation={3} sx={{ width: "100%", mb: 2 }}>
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            {/* <a target='_blank' href={row?.category_image?.image_url} > */}
                            <img
                          ...
                          {row.category_name ? (
                            <TableCell
                              onClick={() => goToEditCategoryPage(row?._id)}
           ...
           ComponentButton={ <VisibilityOutlinedIcon style={{cursor:"pointer"}} onClick={()=>setCategoryIdForEdit(row._id)} fontSize='small' />} /> */}
                              ...
                              {/* {row.category_name &&  <AppRegistrationIcon style={{cursor:'pointer'}} fontSize='small' onClick={()=>{setCategoryIdForEdit(row._id);setDrawerEditCategory(true) }} />} */}
                              {row.category_name && (
                                <AppRegistrationIcon
                                  style={{ cursor: "pointer" }}
                                  fontSize="small"
