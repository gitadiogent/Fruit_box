...
const createShiprocketOrder = async (orderDetails, token) => {
  // console.log("ShiprocketorderDetails--->>>>", orderDetails);

  const order_items = [];
  orderDetails?.products?.map((value) => {
    order_items?.push({
      name: `${value?.product_name} ${
        value?.selected_variation && value?.selected_variation[0]
          ? ` , ${value?.selected_variation[0]}`
          : ""
      } ${
        value?.selected_variation && value?.selected_variation[1]
  ...
  const data = JSON.stringify({
    order_id: JSON.stringify(orderDetails?.order_id).replace("#", ""),
    ...
    sub_total:
      parseInt(orderDetails?.order_total) -
      parseInt(orderDetails?.delivery_charges),

    shipping_charges: orderDetails?.delivery_charges,
