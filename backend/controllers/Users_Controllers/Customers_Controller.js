...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!findUserEmail) {
      findUserPhone = await Customers_Schema.findOne({
        phone_number: phone_number,
      });
      if (!findUserPhone) {
  ...
  try {
    ...
    if (searchValue) {
      result = await Customers_Schema.find({
        ...
        .limit(limit)
        .skip((page - 1) * limit);
      getAllUserCount = await Customers_Schema.find({
        username: { $regex: searchRegex },
      }).count();
      if (!result.length > 0) {
...
const editUserByID = async (req, res) => {
  ...
  try {
    ...
    if (req.body.password) {
      ...
      const find = await Customers_Schema.findByIdAndUpdate(userId, {
    ...
    const findUser = await Customers_Schema.findByIdAndUpdate(userId, {
      $set: req.body,
    });
    if (!findUser) {
      return res.send("user not found");
