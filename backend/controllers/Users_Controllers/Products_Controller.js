...
const createProducts = async (req, res) => {
  try {
    ...
    const checkLimit = await Details_Schema.findOne({
      user_type: "admin",
    }).select("product_limit");
    console.log(
      "checkLimit===>",
      checkLimit?.product_limit,
      "getProductsCount===>",
      getProductsCount
    );
    if (checkLimit?.product_limit < getProductsCount) {
      return res
        .status(404)
        .send({ status: false, message: "you reached product limit!!" });
    }
    ...
    const create = new Products_Schema({
      ...
      product_slug: Utils.splitString(req.body.product_name?.toLowerCase()),
      product_code: req.body.product_code,
      product_regular_price: req.body.product_regular_price,
      product_sale_price: req.body.product_sale_price,
      // product_quantity:req.body.product_quantity,
      product_original_quantity: req.body.product_original_quantity,
      ...
      product_description_website: req.body.product_description_website,
  ...
  try {
    ...
    if (date_from && date_to && by_category) {
      if (by_category != "all") {
        ...
        return res.status(200).send({
          allProducts: result,
          pages: Math.ceil(count[0]?.product_count / limit),
          getProductsCount: count[0]?.product_count,
...
const deleteProductReview = async (req, res) => {
  const productId = req.params.product_id;
  const productReviewId = req.params.review_id;
  console.log("productId , productReviewId =>", productId, productReviewId);
  try {
    if (!productId) {
      return res
        .status(404)
    ...
    let result = findProduct;
    // console.log("result=>",result)
    const filteredProductsRev = await result?.product_review?.filter(
      (value, index) => value?._id != productReviewId
    );
    console.log("result----->", filteredProductsRev);
    const updateProductProductReview = await Products_Schema.findByIdAndUpdate(
      productId,
      { $set: { product_review: filteredProductsRev } }
    );
    // console.log("updated products=>",updateProductProductReview);
    res.status(200).send({
      status: true,
    ...
    console.log(err);
    res.status(200).send({ status: false, message: "review delete failed !!" });
  }
};
