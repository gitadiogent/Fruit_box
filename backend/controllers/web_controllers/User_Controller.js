...
// creating new user by google
const createUser_website_google_single = async (req, res) => {
  const {
    firstname,
    gst_number,
    pincode,
  ...
  try {
    ...
    const token = await Utils.create_Jwt(
      { id: result._id, user_type: result.user_type },
      config.JWT_TOKEN_SECRET
    );
...
// creating new user by google
const createUser_website = async (req, res) => {
  const {
    firstname,
    gst_number,
    pincode,
...
const registerUserLocalStorage = async (req, res) => {
  const username = req.body.username;
  const phone_number = req.body.phone_number;
  try {
    if (!username || !phone_number) {
...
const registerVerifylocalStorage = async (req, res) => {
  try {
    const { otp } = req.body;
    const { phone_number } = req.body;
    const { wlUser } = req.body;
...
const loginUserByNumberLocalStorage = async (req, res) => {
  try {
    const { phone_number } = req.body;
    if (!phone_number) {
      return res.send("Please Enter a Phone number !!");
...
const verifyOTPRegLogLocalStorage = async (req, res) => {
  try {
    const { otp } = req.body;
    ...
    const user = findUser;
    res.send({ user, success: true, jwt: token });
...
// OTP Login
const verifyOTPRegLogGoogle = async (req, res) => {
  try {
    const { email } = req.body;

    if (!email) {
      return res.send({ message: "Please proide Email !!!", success: false });
    }
...
const editUserByID = async (req, res) => {
  ...
  try {
    ...
    findUser.phone_number = req.body.phone_number;
    findUser.address = req.body.address;
    findUser.email = req.body.email;
    findUser.pincode = req.body.pincode;
...
const googleLoginNodeJs = async (req, res) =>
...
exports.loginUserByNumberLocalStorage = loginUserByNumberLocalStorage;
exports.verifyOTPRegLogLocalStorage = verifyOTPRegLogLocalStorage;
exports.googleLoginNodeJs = googleLoginNodeJs;
exports.createUser_website_google_single = createUser_website_google_single;
exports.verifyOTPRegLogGoogle = verifyOTPRegLogGoogle;
exports.createUser_website = createUser_website;
...
exports.registerUserLocalStorage = registerUserLocalStorage;
exports.registerVerifylocalStorage = registerVerifylocalStorage;
