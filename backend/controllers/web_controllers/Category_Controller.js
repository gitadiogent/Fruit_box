...
const Catgeory_Schema = require("../../modals/UserModals/Category");
const { createRegex } = require("../../utils/Utils");

// get all brands
const getAllBrands = async (req, res) => {
  try {
    const result = await Catgeory_Schema.find();
...
const getCategorybyName = async (req, res) => {
  try {
    ...
    const result = await Catgeory_Schema.find({
      main_category_name: req.params.name,
    });

    let newArr = result.map((ele) => {
...
const createCategory = async (req, res) => {
  ...
  try {
    const findExistingCategory = await Catgeory_Schema.find({
    ...
    const findMainCategory = await Catgeory_Schema.find({
      main_category_name: req.body.mainCategory,
    });
    console.log(findMainCategory);
    if (findMainCategory[0]?.category_name) {
      console.log("FIND SUCCESS=>", findMainCategory[0]?.category_name);

      const create = new Catgeory_Schema({
      ...
      const updateCategory = await Catgeory_Schema.findByIdAndUpdate(
        findMainCategory[0]?._id,
        {
          $set: {
            main_category_name: req.body.mainCategory?.toLowerCase(),
            main_category_slug: req.body.main_category_slug?.toLowerCase(),
...
const createMainCategory = async (req, res) => {
  try {
    ...
    if (findExistingCategory.length > 0) {
      return res
        .status(200)
        .send({ status: false, message: "main category already exists !!" });
    }
    const create = new Catgeory_Schema({
      main_category_name: req.body.mainCategory?.toLowerCase(),
      main_category_slug: req.body.main_category_slug?.toLowerCase(),
...
const editCategory = async (req, res) => {
  ...
  try {
    ...
    const findCategory = await Catgeory_Schema.findByIdAndUpdate(categoryId, {
      $set: req.body,
    });
    if (!findCategory) {
...
const updateMainCategory = async (req, res) => {
  ...
  try {
    if (oldMainCategory) {
      const find = await Catgeory_Schema.find({
        main_category_name: oldMainCategory,
      });
      console.log("FIND___", find);
      if (find?.length > 0) {
        const updateAll = await Catgeory_Schema.updateMany(
          { main_category_name: oldMainCategory },
          {
            $set: {
...
const deleteImage = async (req, res) => {
  ...
  try {
    if (oldMainCategory) {
      ...
      if (find?.length > 0) {
        const removeImage = await Catgeory_Schema.updateMany(
          { main_category_name: oldMainCategory },
          {
            $set: {
              main_category_image: {},
        ...
        return res
          .status(200)
          .send({ status: true, message: "image deleted success !!" });
      }
    }
    if (oldCategory) {
      const findCategory = await Catgeory_Schema.find({
      ...
      if (findCategory?.length > 0) {
        ...
        return res
          .status(200)
          .send({ status: true, message: "image delete success !!" });
      }
    }
    if (oldSubId && oldSubCategory) {
      console.log("old sub category=>", oldSubId, "--", oldSubCategory);
      const findSubCategroy = await Catgeory_Schema.find({
        _id: oldSubId,
      ...
      if (findSubCategroy?.length > 0) {
        ...
        await result?.map((value, index) => {
          ...
          if (value.name === oldSubCategory) {
            // console.log("ENTERED")
            result[index].image = { image_name: "", image_url: "", path: "" };
          }
          // console.log("EXIT")
        });
        // console.log("resutl=>",result)

        const updateSubCategory = await Catgeory_Schema.findByIdAndUpdate(
          { _id: oldSubId },
          { $set: { subcategory: result } }
        );
...
const getAllCategory = async (req, res) => {
  try {
    const countCategory = await Catgeory_Schema.find({}).count();
    const categoryForFilter = await Catgeory_Schema.aggregate([
      { $group: { _id: "$main_category_name" } },
    ]).sort({ _id: 1 });
    //  console.log(categoryForFilter)
    const allCategory = await Catgeory_Schema.find({});
    res.status(200).send({
...
const getCategoryById = async (req, res) => {
  ...
  try {
    ...
    const getCategoires = await Catgeory_Schema.findById(cat_id);
    if (!getCategoires) {
...
const filterForCategory = async (req, res) => {
  ...
  try {
    if (main_category) {
      result = await Catgeory_Schema.find({
...
const searchInCategory = async (req, res) => {
  ...
  try {
    const countCategory = await Catgeory_Schema.find({}).count();
    result = await Catgeory_Schema.find({
      main_category_name: { $regex: searchRegex },
    });
    if (!result.length) {
      result = await Catgeory_Schema.find({
    ...
    const categoryForFilter = await Catgeory_Schema.aggregate([
      {
        $group: {
          ...
          main_category_image: { $first: "$main_category_image" },
        },
      },
      {
        $project: {
...
const getCategoryByMainCategory = async (req, res) => {
  ...
  try {
    if (main_category) {
      const findCategory = await Catgeory_Schema.aggregate([
    ...
    // if(categoryID){
    //     const findSubCategroy= await Catgeory_Schema.findById(categoryID)
...
const deleteCategory = async (req, res) => {
  ...
  try {
    if (req.body?.length) {
      const deleteSelected = await Catgeory_Schema.deleteMany({
        _id: {
          $in: req.body,
        },
      });
