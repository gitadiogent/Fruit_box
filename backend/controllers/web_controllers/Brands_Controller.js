...
const getAllBrands = async (req,res) => {
  try {
    const result = await Category_Schema.find();
...
const createCategory = async (req, res) => {
  ...
  try {
    const findExistingCategory = await Category_Schema.find({
    ...
    const findMainCategory = await Category_Schema.find({
    ...
    if (findMainCategory[0]?.category_name) {
      ...
      const updateCategory = await Category_Schema.findByIdAndUpdate(
...
const editCategory = async (req, res) => {
  ...
  try {
    ...
    const findCategory = await Category_Schema.findByIdAndUpdate(categoryId, {
      $set: req.body,
    });
    if (!findCategory) {
...
const updateMainCategory = async (req, res) => {
  ...
  try {
    if (oldMainCategory) {
      ...
      if (find?.length > 0) {
        const updateAll = await Category_Schema.updateMany(
          { main_category_name: oldMainCategory },
          {
            $set: {
...
const deleteImage = async (req, res) => {
  ...
  try {
    if (oldMainCategory) {
      ...
      if (find?.length > 0) {
        const removeImage = await Category_Schema.updateMany(
          { main_category_name: oldMainCategory },
          {
            $set: {
              main_category_image: {},
    ...
    if (oldCategory) {
      const findCategory = await Category_Schema.find({
    ...
    if (oldSubId && oldSubCategory) {
      ...
      const findSubCategroy = await Category_Schema.find({
        _id: oldSubId,
      ...
      if (findSubCategroy?.length > 0) {
        ...
        const updateSubCategory = await Category_Schema.findByIdAndUpdate(
          { _id: oldSubId },
...
const getCategoryById = async (req, res) => {
  ...
  try {
    ...
    const getCategoires = await Category_Schema.findById(cat_id);
    if (!getCategoires) {
...
const filterForCategory = async (req, res) => {
  ...
  try {
    if (main_category) {
      result = await Category_Schema.find({ main_category_name: main_category });
      console.log(result);
...
const searchInCategory = async (req, res) => {
  ...
  try {
    const countCategory = await Category_Schema.find({}).count();
    result = await Category_Schema.find({
...
const getCategoryByMainCategory = async (req, res) => {
  ...
  try {
    if (main_category) {
      const findCategory = await Category_Schema.aggregate([
    ...
    // if(categoryID){
    //     const findSubCategroy= await Category_Schema.findById(categoryID)
...
const deleteCategory = async (req, res) => {
  ...
  try {
    if (req.body?.length) {
      const deleteSelected = await Category_Schema.deleteMany({
        _id: {
          $in: req.body,
        },
      });
